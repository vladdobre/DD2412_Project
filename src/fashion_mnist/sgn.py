...
flags.DEFINE_integer(
    "ood_interval", 0, "Interval to run OOD evaluation on specified OOD datasets."
)
flags.DEFINE_list(
    "ood_dataset", "cifar100,svhn_cropped", "list of OOD datasets to evaluate on."
)
flags.DEFINE_string(
    "saved_model_dir", None, "Directory containing the saved model checkpoints."
)
flags.DEFINE_bool(
    "dempster_shafer_ood", False, "Wheter to use DempsterShafer Uncertainty score."
)


# Noisy labels flags.
flags.DEFINE_bool(
    "noisy_labels", False, "Whether to run only eval and (maybe) OOD steps."
)
flags.DEFINE_float("severity", 0.0, "Noise ratio..")
flags.DEFINE_enum(
    "corruption_type",
    "asym",
    enum_values=[
...
def get_smoothed_onehot(labels, num_classes):
    ...
    smoothed_targets = (
        1.0 - FLAGS.label_smoothing
    ) * one_hot_labels + FLAGS.label_smoothing * tf.ones(
        tf.shape(one_hot_labels)
    ) / float(dim)
...
def helmert_tf(n):
    tensor = tf.ones((n, n))
    H = tf.linalg.set_diag(
        tf.linalg.band_part(tensor, -1, 0), 1 - tf.range(1, n + 1, dtype=tf.float32)
    )
...
def main(argv):
    ...
    # Use GPU
    # strategy = tf.distribute.MirroredStrategy(devices=[0])
    ...
    # )
    train_builder = ub.datasets.FashionMnistDataset(
        # FLAGS.dataset,
        # data_dir=data_dir,
        download_data=FLAGS.download_data,
        split=tfds.Split.TRAIN,
    ...
    if FLAGS.noisy_labels:
        ...
        dataset_cls = ub.datasets.FashionMnistDataset
    ...
    if FLAGS.train_proportion < 1.0:
        validation_builder = ub.datasets.FashionMnistDataset(
    ...
    clean_test_builder = ub.datasets.FashionMnistDataset(
    ...
    if FLAGS.corruptions_interval > 0:
        if FLAGS.dataset == "cifar100":
            data_dir = FLAGS.fashion_mnist_data_dir
        corruption_types, _ = utils.load_corrupted_test_info(FLAGS.dataset)
        for corruption_type in corruption_types:
            for severity in range(1, 6):
                dataset = ub.datasets.FashionMnistDataset(
    ...
    with strategy.scope():
        ...
        optimizer = tf.keras.optimizers.SGD(
            base_lr, momentum=1.0 - FLAGS.one_minus_momentum, nesterov=True
        )

        metrics = {
            "train/accuracy_nl": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/negative_log_likelihood_c10n": tf.keras.metrics.Mean(),
            "train/accuracy_tl_clean": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/accuracy_tl_corrupted": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/accuracy_nl_corrupted": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/negative_log_likelihood": tf.keras.metrics.Mean(),
            "train/accuracy": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/accuracy_tl": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/accuracy_delta_tl_clean": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/accuracy_delta_tl_corrupted": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/accuracy_delta_nl_corrupted": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/accuracy_delta": tf.keras.metrics.SparseCategoricalAccuracy(),
            "train/accuracy_delta_tl": tf.keras.metrics.SparseCategoricalAccuracy(),
            ...
            "test/negative_log_likelihood_delta": tf.keras.metrics.Mean(),
            "test/accuracy_delta": tf.keras.metrics.SparseCategoricalAccuracy(),
            "test/ece_delta": rm.metrics.ExpectedCalibrationError(
    ...
    def train_step(iterator, epoch):
        ...
        def step_fn(inputs, step):
            ...
            exponent = (
                tf.cast(epoch, tf.float32) + tf.cast(step, tf.float32) / steps_per_epoch
            )

            with tf.GradientTape(persistent=True) as tape:
                loc, scale = model(images, training=True)

                normal = _create_normal(
                    loc, scale, loc_ema, num_classes, smoothed_targets, exponent
                )
            ...
            with tf.control_dependencies([opt_op]):
                ...
                for v, v_delta in zip(
                    model.trainable_variables, model_delta.trainable_variables
                ):
            ...
            metrics["train/accuracy_delta_tl"].update_state(
                inputs["labels"], probs_delta
            )

            if FLAGS.noisy_labels:
                mask = inputs["labels"] == inputs["noisy_labels"]
                tl_labels_clean, tl_labels_corrupted = (
                    tf.boolean_mask(inputs["labels"], mask),
                    tf.boolean_mask(inputs["labels"], ~mask),
                )
                nl_labels_corrupted = tf.boolean_mask(inputs["noisy_labels"], ~mask)
                probs_clean, probs_corrupted = (
                    tf.boolean_mask(probs, mask),
                    tf.boolean_mask(probs, ~mask),
                )
                ...
                probs_delta_clean, probs_delta_corrupted = (
                    tf.boolean_mask(probs_delta, mask),
                    tf.boolean_mask(probs_delta, ~mask),
                )

                # Accuracy
                metrics["train/accuracy_delta_tl_clean"].update_state(
                    tl_labels_clean, probs_delta_clean
                )
                metrics["train/accuracy_delta_tl_corrupted"].update_state(
                    tl_labels_corrupted, probs_delta_corrupted
                )
                metrics["train/accuracy_delta_nl_corrupted"].update_state(
                    nl_labels_corrupted, probs_delta_corrupted
                )
    ...
    def test_step(iterator, dataset_split, dataset_name, num_steps, model_eval, suffix):
        ...
        def step_fn(inputs):
            ...
            labels_noisy = (
                inputs["noisy_labels"]
                if FLAGS.noisy_labels and dataset_split in ["train", "validation"]
                else inputs["labels"]
            )
            ...
            if dataset_name == "clean":
                if dataset_split == "validation":
                    metrics[
                        f"{dataset_split}/negative_log_likelihood" + suffix
                    ].update_state(negative_log_likelihood)
                    metrics[f"{dataset_split}/accuracy_tl" + suffix].update_state(
                        labels, probs
                    )
                    metrics[f"{dataset_split}/accuracy_nl" + suffix].update_state(
                        labels_noisy, probs
                    )
                    metrics[f"{dataset_split}/ece" + suffix].add_batch(
                        probs, label=labels
                    )
                else:
                    metrics[
                    ...
                    metrics[f"{dataset_split}/accuracy" + suffix].update_state(
                        labels, probs
                    )
                    metrics[f"{dataset_split}/ece" + suffix].add_batch(
                        probs, label=labels
                ...
                dataset_name == "train_c10n" and FLAGS.noisy_labels and FLAGS.eval_only
            ):
                assert False
                rand1, rand2, rand3 = (
                    inputs["label_rand1"],
                ...
                nll_vec = (
                    tf.keras.losses.sparse_categorical_crossentropy(rand1, probs)
    ...
    if FLAGS.collect_profile:
        tb_callback = tf.keras.callbacks.TensorBoard(
            profile_batch=(100, 102), log_dir=os.path.join(FLAGS.output_dir, "logs")
        )
    ...
    for epoch in range(initial_epoch, FLAGS.train_epochs):
        ...
        for model_eval, suffix in zip(models, suffixes):
            ...
            if FLAGS.ood_interval > 0 and (epoch + 1) % FLAGS.ood_interval == 0:
