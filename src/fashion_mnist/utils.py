...
flags.DEFINE_enum(
    "dataset",
    "fashion_mnist",
    enum_values=["fashion_mnist", "cifar10"],
    help="Dataset.",
)
flags.DEFINE_string(
    "data_dir",
...
flags.DEFINE_bool(
    "download_data",
    False,
    "Whether to download data locally when initializing a " "dataset.",
)
...
flags.DEFINE_integer(
    "shuffle_buffer_size", None, "Shuffle buffer size for training dataset."
)
flags.DEFINE_integer("seed", 42, "Random seed.")
flags.DEFINE_integer("train_epochs", 600, "Number of training epochs.")
flags.DEFINE_float(
    "train_proportion", default=0.9, help="only use a proportion of training set."
)
...
# Architecture
flags.DEFINE_integer("width", 2, "Width of ResNet.")
...
def load_fashion_mnist(
    ...
    if use_bfloat16:
        ...
        dtype = tf.float32
    filename = path + "{0}-{1}.tfrecords".format(corruption_name, corruption_intensity)
    ...
    def preprocess(serialized_example):
        ...
        image = tf.cast(tf.reshape(image, [28, 28, 1]), dtype)
        image = tf.image.convert_image_dtype(image, dtype)
        image = image / 255  # to convert into the [0, 1) range
        if normalize:
            mean = tf.constant([0.2860], dtype=dtype)
            std = tf.constant([0.3530], dtype=dtype)
...
#     results = {
#         "{}/nll_mean_corrupted".format(prefix): 0.0,
#         "{}/kl_mean_corrupted".format(prefix): 0.0,
#         "{}/elbo_mean_corrupted".format(prefix): 0.0,
#         "{}/accuracy_mean_corrupted".format(prefix): 0.0,
#         "{}/ece_mean_corrupted".format(prefix): 0.0,
#         "{}/member_acc_mean_corrupted".format(prefix): 0.0,
#         "{}/member_ece_mean_corrupted".format(prefix): 0.0,
#     }
#     fine_metrics_results = {}
#     if corrupt_diversity is not None:
#         for key in diversity_keys:
#             results["corrupt_diversity/{}_mean_corrupted".format(key)] = 0.0
#
...
#             # this be cleaned up?
#             ece[i] = list(
#                 metrics["{0}/ece_{1}".format(prefix, dataset_name)].result().values()
#             )[0]
#             if "{0}/member_acc_mean_{1}".format(prefix, dataset_name) in metrics.keys():
#                 member_acc[i] = metrics[
#                     "{0}/member_acc_mean_{1}".format(prefix, dataset_name)
#                 ].result()
#             else:
#                 member_acc[i] = 0.0
#             if "{0}/member_ece_mean_{1}".format(prefix, dataset_name) in metrics.keys():
#                 member_ece[i] = list(
#                     metrics["{0}/member_ece_mean_{1}".format(prefix, dataset_name)]
#                     .result()
#                     .values()
#                 )[0]
#             else:
#                 member_ece[i] = 0.0
#             if corrupt_diversity is not None:
#                 disagreement[i] = corrupt_diversity[
#                     "corrupt_diversity/disagreement_{}".format(dataset_name)
#                 ].result()
#                 # Normalize the corrupt disagreement by its error rate.
#                 error = 1 - acc[i] + tf.keras.backend.epsilon()
#                 cosine_similarity[i] = (
#                     corrupt_diversity[
#                         "corrupt_diversity/cosine_similarity_{}".format(dataset_name)
#                     ].result()
#                 ) / error
#                 average_kl[i] = corrupt_diversity[
#                     "corrupt_diversity/average_kl_{}".format(dataset_name)
#                 ].result()
#             if log_fine_metrics or output_dir is not None:
#                 fine_metrics_results["{0}/nll_{1}".format(prefix, dataset_name)] = nll[
#                     i
#                 ]
#                 fine_metrics_results["{0}/kl_{1}".format(prefix, dataset_name)] = kl[i]
#                 fine_metrics_results["{0}/elbo_{1}".format(prefix, dataset_name)] = (
#                     elbo[i]
#                 )
#                 fine_metrics_results[
#                     "{0}/accuracy_{1}".format(prefix, dataset_name)
#                 ] = acc[i]
#                 fine_metrics_results["{0}/ece_{1}".format(prefix, dataset_name)] = ece[
#                     i
#                 ]
#                 if corrupt_diversity is not None:
#                     fine_metrics_results[
...
#         results["{0}/accuracy_median_{1}".format(prefix, intensity)] = np.median(acc)
#         results["{0}/ece_median_{1}".format(prefix, intensity)] = np.median(ece)
#         results["{0}/member_acc_mean_{1}".format(prefix, intensity)] = avg_member_acc
#         results["{0}/member_ece_mean_{1}".format(prefix, intensity)] = avg_member_ece
...
#     if corrupt_diversity is not None:
#         for key in diversity_keys:
#             results["corrupt_diversity/{}_mean_corrupted".format(key)] /= max_intensity
#
