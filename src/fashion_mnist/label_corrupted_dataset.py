...
def get_element_id(example_id):
    tf.debugging.Assert(tf.strings.substr(example_id, 0, 6) == "train_", [example_id])
...
def noisify_instance(train_data, train_labels, noise_rate):
    ...
    over_all_noise_rate = (
        1 - float((train_labels == noisy_labels).sum()) / train_labels.shape[0]
    )
    ...
    class _LabelCorruptedBaseDataset(dataset_cls):
        ...
        def __init__(
            ...
            **kwargs,
            ...
            is_fashion_mnist = dataset.name == "fashion_mnist"
            self.num_classes = 10 if is_c10 else 100

            # assert not (not is_c10 and self.is_synthetic)
            if not is_c10:
                assert corruption_type in ["c100noise", "sym", "asym", "instance"]
            ...
            if is_fashion_mnist:
                paths_labels = (
                        "./src/cifar/datasets/CIFAR-10_human_ordered.npy"
                    if is_c10
                    else "./src/cifar/datasets/CIFAR-100_human_ordered.npy"
                )
                paths_indices = (
                    "./src/cifar/datasets/image_order_c10_inverted.npy"
                    if is_c10
                    else "./src/cifar/datasets/image_order_c100_inverted.npy"
                )
                ...
                feature_description = {
                    "image": tf.io.FixedLenFeature([], tf.string),  # Serialized image
                    "label": tf.io.FixedLenFeature([], tf.int64),  # Integer label
                }

                # Define the parsing function to decode serialized examples
                ...
                file_path = "./src/fashion_mnist/datasets/fashion_mnist/3.0.1/fashion_mnist-train.tfrecord-00000-of-00001"
                ...
                labels = []
                indexes = []
                for idx, label in enumerate(parsed_dataset):
                    labels.append(label.numpy())
                    indexes.append(idx)
                labels = np.array(labels)
                indexes = np.array(indexes)
                self.clean_labels = tf.convert_to_tensor(np.array(labels))
                self.index_map = tf.convert_to_tensor(indexes)
                ...
                # corruption_type_to_noise_key = {
                #     "worst": "worse_label",
                #     "aggre": "aggre_label",
                #     "rand1": "random_label1",
                #     "rand2": "random_label2",
                #     "rand3": "random_label3",
                #     "c100noise": "noise_label",
                # }

            self.extra_labels = None
            if is_c10:
                rand1 = tf.cast(
                    tf.convert_to_tensor(noise_file.item().get("random_label1")),
                    tf.float32,
                )
                rand2 = tf.cast(
                    tf.convert_to_tensor(noise_file.item().get("random_label2")),
                    tf.float32,
                )
                rand3 = tf.cast(
                    tf.convert_to_tensor(noise_file.item().get("random_label3")),
                    tf.float32,
                )
                self.extra_labels = {"rand1": rand1, "rand2": rand2, "rand3": rand3}
            ...
            if corruption_type in corruptions_real:
                ...
                id_to_index = np.zeros(10000, dtype=np.int64)
                for v in range(len(element_ids)):
                    id_to_index[element_ids[v]] = np.where(
                        element_ids == element_ids[v]
                    )[0][0]
        ...
        def load(self, *, preprocess_fn=None, batch_size: int = -1) -> tf.data.Dataset:
            if preprocess_fn:
                dataset_preprocess_fn = preprocess_fn
            else:
                dataset_preprocess_fn = self.dataset._create_process_example_fn()  # pylint: disable=protected-access
...
def _create_asym_noisy_label_fn(
    num_classes,
    seed,
    severity,
    id_map=None,
    ...
    def _flip_label_fm(example: types.Features) -> types.Features:
    ...
    def _add_noisy_label(example: types.Features) -> types.Features:
        ...
        # Hack to make fashion_mnist work without "element_id"
        features_hash = tf.strings.to_hash_bucket_fast(
            tf.strings.as_string(tf.reduce_sum(example["features"])), 1_000_000
        )
        element_id = tf.cast(features_hash, tf.int64)
        example["element_id"] = element_id

        per_example_seed = tf.random.experimental.stateless_fold_in(
            seed, example["element_id"]
        )
        ...
        example["noisy_labels"] = tf.cond(
            flip_cond,
            lambda: _flip_label_fm(example),
        ...
        if num_classes == 10:
            ...
            tf.debugging.Assert(
                tf.strings.substr(element_id, 0, 6) == "train_", [element_id]
            )
...
def _create_uniform_noisy_label_fn(
    num_classes,
    seed,
    severity,
    id_map=None,
    ...
    def _flip_label(example: types.Features, random_func) -> types.Features:
        return float(
            random_func(shape=[], minval=0, maxval=num_classes, dtype=tf.int32)
        )
    ...
    def _add_noisy_label(example: types.Features) -> types.Features:
        ...
        example["noisy_labels"] = tf.cond(
            flip_cond,
            lambda: _flip_label(example, random_func),
            lambda: example["labels"],
        )
...
# def _create_real_noisy_label_fn(
#     num_classes, seed, severity, id_map, clean_labels, noisy_labels, extra_labels=None
# ) -> PreProcessFn:
#     """Returns a function that adds an `noisy_labels` key to examles."""
#
#     def _add_noisy_label(example: types.Features) -> types.Features:
...
# def _create_instance_noisy_label_fn(
#     num_classes,
#     seed,
#     severity,
#     id_map=None,
