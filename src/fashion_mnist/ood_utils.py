...
def create_ood_metrics(ood_dataset_names):
    ...
    for dataset_name in ood_dataset_names:
        ood_metrics.update(
            {
                "ood/auroc_{}".format(dataset_name): tf.keras.metrics.AUC(
                    curve="ROC", num_thresholds=100000
                ),
                "ood/auprc_{}".format(dataset_name): tf.keras.metrics.AUC(
                    curve="PR", num_thresholds=100000
                ),
                "ood/(1-fpr)@95tpr_{}".format(
                    dataset_name
