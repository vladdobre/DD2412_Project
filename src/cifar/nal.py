...
flags.DEFINE_float("label_smoothing", 0.0, "Label smoothing parameter in [0,1].")
...
flags.DEFINE_bool("augmix", False, "Whether to perform AugMix [4] on the input data.")
...
flags.DEFINE_integer(
    "augmix_width",
    3,
    "Augmix width. This corresponds to the k in line 5 in the" "Algorithm box in [4].",
)
...
flags.DEFINE_float("alpha", 0.98, "momentum parameter.")
flags.DEFINE_float(
    "reg_scale", 10.0, "regularization scale parameter (lambda in the paper)."
)
...
flags.DEFINE_bool(
    "eval_on_ood", False, "Whether to run OOD evaluation on specified OOD datasets."
)
flags.DEFINE_list(
...
def _generalized_energy_distance(labels, predictions, num_classes):
    ...
    sample_diversity = tf.reduce_sum(
        tf.reduce_sum(non_diag * y_hat * tf.transpose(y_hat, perm=[0, 2, 1]), -1), -1
    )
...
def main(argv):
    ...
    def train_step(iterator, preds_buffer):
        ...
        def step_fn(inputs, preds_buffer):
            ...
            with tf.GradientTape() as tape:
                ...
                preds_buffer_update = (
                    FLAGS.alpha * preds_buffer_update + (1 - FLAGS.alpha) * preds_
                )
                preds_buffer_update = tf.clip_by_value(preds_buffer_update, 1e-4, 1)
                ...
                negative_log_likelihood = tf.reduce_mean(
                    tf.keras.losses.categorical_crossentropy(
                        preds_buffer_update, preds_combined, from_logits=False
                    )
                )
    ...
    def test_step(iterator, dataset_split, dataset_name, num_steps):
        ...
        def step_fn(inputs):
            ...
            labels_noisy = (
                inputs["noisy_labels"]
                if FLAGS.noisy_labels and dataset_split == "validation"
                else inputs["labels"]
            )
    ...
    preds_buffer, max_indices = buffer_init_step(train_iterator, preds_buffer)
    assert max_indices < preds_buffer.shape[0], (
        f"there is an error in indexing preds_buffer.shape[0] must be bigger "
    ...
    for epoch in range(initial_epoch, FLAGS.train_epochs):
        ...
        if not FLAGS.eval_only:
            ...
            preds_buffer, max_indices = train_step(train_iterator, preds_buffer)
            assert max_indices < preds_buffer.shape[0], (
                f"there is an error in indexing preds_buffer.shape[0] must be bigger "
                f"than max_indices but {preds_buffer.shape[0]}<{max_indices}"
            )
