...
def main(argv):
    ...
    def train_step(iterator):
        ...
        def step_fn(inputs):
            ...
            with tf.GradientTape() as tape:
                ...
                V_square = tf.gather(buffer_v, indices) ** 2 * (1.0 - one_hot_labels)
                ...
                prediction = prediction / tf.math.maximum(
                    tf.norm(prediction, ord=1), eps
                )
                prediction = tf.clip_by_value(prediction, eps, 1.0)

                pred_one_hot = tf.one_hot(
                    tf.math.argmax(
                        tf.stop_gradient(logits), axis=1, output_type=tf.int32
                    ),
                    num_classes,
                )
                MSE_loss = tf.reduce_mean(
                    tf.reduce_sum(
                        tf.square(
                            one_hot_labels - (pred_one_hot + U_square - V_square)
                        ),
                        axis=1,
                    )
                )
    ...
    # SOP Buffers
    buffer_u = tf.Variable(tf.random.normal((50000, 1), stddev=1e-8), dtype=tf.float32)
    buffer_v = tf.Variable(
        tf.random.normal((50000, num_classes), stddev=1e-8), dtype=tf.float32
    )
