...
flags.DEFINE_float(
    "min_scale", 1.0, "Added constant to the predicted scale parameters."
)
flags.DEFINE_float(
    "max_scale", 1.0, "Added constant to the predicted scale parameters."
)
flags.DEFINE_bool(
    "mu_as_pred", False, "Whether to use mu instead of average of samples as pred."
)
...
def _create_logitnormal(loc, scale, min_scale, num_classes, labels=None):
    ...
    logit_normal = tfd.TransformedDistribution(
        distribution=mvn, bijector=bijector, name="LogitNormalTransformedDistribution"
    )
...
def main(argv):
    ...
    with strategy.scope():
        ...
        metrics = {
            ...
            "train/entropy_clean": tf.keras.metrics.Mean(),
            "train/entropy_noisy": tf.keras.metrics.Mean(),
            "train/mse_mu_label_clean": tf.keras.metrics.Mean(),
            "train/mse_mu_label_noisy": tf.keras.metrics.Mean(),
            "train/grad_scale": tf.keras.metrics.Mean(),
            "train/grad_mu": tf.keras.metrics.Mean(),
            "train/grad_scale_clean": tf.keras.metrics.Mean(),
            "train/grad_scale_noisy": tf.keras.metrics.Mean(),
            "train/grad_mu_clean": tf.keras.metrics.Mean(),
            "train/grad_mu_noisy": tf.keras.metrics.Mean(),
        ...
        def step_fn(inputs):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                logit_normal = _create_logitnormal(
                    loc, scale, min_scale, num_classes, smoothed_targets
                )
            ...
            if FLAGS.noisy_labels:
                ...
                entropy_c, entropy_n = (
                    tf.reduce_mean(tf.boolean_mask(entropy, mask)),
                    tf.reduce_mean(tf.boolean_mask(entropy, ~mask)),
                )
                ...
                mse_mu_c, mse_mu_n = (
                    tf.reduce_mean(tf.boolean_mask(mse_mu, mask)),
                    tf.reduce_mean(tf.boolean_mask(mse_mu, ~mask)),
                )
                g_mu_c, g_mu_n = (
                    tf.reduce_sum(tf.boolean_mask(grads_mu, mask)),
                    tf.reduce_sum(tf.boolean_mask(grads_mu, ~mask)),
                )
                g_scale_c, g_scale_n = (
                    tf.reduce_sum(tf.boolean_mask(grads_scale, mask)),
                    tf.reduce_sum(tf.boolean_mask(grads_scale, ~mask)),
                )
                ...
                metrics["train/grad_scale_clean"].update_state(
                    tf.reduce_mean(g_scale_c)
                )
                metrics["train/grad_scale_noisy"].update_state(
                    tf.reduce_mean(g_scale_n)
                )
