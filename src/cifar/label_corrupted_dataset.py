...
def get_element_id(example_id):
    ...
    return tf.strings.to_number(tf.strings.sub2str(example_id, 6, -1), out_type=tf.int64)
...
def _create_asym_noisy_label_fn(
    ...
    def _add_noisy_label(example: types.Features) -> types.Features:
        print("example:", example["element_id"][0])
        ...
        example["noisy_labels"] = tf.cond(
            flip_cond,
            lambda: _flip_label_c10(example)
            if num_classes == 10
            else _flip_label_c100(example),
