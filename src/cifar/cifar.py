...
class _CifarDataset(base.BaseDataset):
    ...
    def _create_process_example_fn(self) -> base.PreProcessFn:
        def _example_parser(example: types.Features) -> types.Features:
            ...
            if self._is_training:
                ...
                def weak_aug(image, seed1, seed2):
                    image = tf.image.stateless_random_crop(
                        image, (image_shape[0], image_shape[0], 3), seed=seed1
                    )
                    image = tf.image.stateless_random_flip_left_right(image, seed=seed2)
                    return image

                if (
                    not self._aug_params.get("random_augment", False)
                    and not use_augmix
                    and self._aug_params["aug_count"] > 1
                ):
                    ...
                    image = weak_aug(
                        image, per_example_step_seeds[0], per_example_step_seeds[1]
                    )
            ...
            label_smoothing = self._aug_params.get("label_smoothing", 0.0)
    ...
    def _create_process_batch_fn(self, batch_size: int) -> Optional[base.PreProcessFn]:
